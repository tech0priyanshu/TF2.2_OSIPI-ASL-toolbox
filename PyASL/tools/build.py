    ...
    StringFileInfo(
      ...
      StringTable(
        ...
        [StringStruct('CompanyName', 'OSIPI TF2.2 Taskforce'),
         ...
         StringStruct('InternalName', 'PyASL-GUI'),
         StringStruct('LegalCopyright', 'Copyright (c) 2026 OSIPI TF2.2 Taskforce'),
         StringStruct('OriginalFilename', 'PyASL-GUI.exe'),
...
def _validate_build_windows(project_root: Path, dist_dir: Path):
    ...
    exe_path = app_dist_dir / "PyASL-GUI.exe"
    if not exe_path.exists():
        raise RuntimeError(f"Validation Failed: Executable not found at {exe_path}")
    logger.info("  ✓ Executable exists: %s", exe_path)
...
def main():
    ...
    # 6. Copy branding assets to dist bundle (Windows flat directory layout)
    if not IS_MACOS:
        app_dist_dir = dist_dir / "PyASL-GUI"
        if app_dist_dir.exists():
    ...
    app_label = dist_dir / ("PyASL-GUI.app" if IS_MACOS else "PyASL-GUI")
    logger.info("=" * 60)
